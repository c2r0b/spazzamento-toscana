...
def update_json_file(city, data):
    ...
    # Find the LUCCA entry and update its 'data' array
...
def extract_info_external(s):
    ...
    weekDay = [day_name_to_number[day_name.upper()]]  # Ensure the day name is uppercase for matching
...
def extract_schedule_external(s, shared_info):
    ...
    #remove \n, - and ; from location
    data['location'] = s.replace('\n', '').replace('-', '').replace(';', '').replace(r'\s\s', ' ').replace(', ', '').replace(':', '').replace('\u2019', "'").split('COMUNE DI LUCCAUProtocollo')[0].strip()
    ...
    for chunk in chunks:
        ...
        for street in streets:
            ...
            if len(locality) > 1:
                ...
                locality = re.split('fraz. ', street)[1].split(':')[0].replace('\n', '').strip().split(';')[0]
            ...
            if len(re.split(r'\n[\s]+-', street)) > 1:
                ...
                for part in street_parts:
                    schedule.append(extract_schedule_external(part, shared_info))
            else:
...
def extract_schedule_center(s, shared_info):
    ...
    # Match the pattern with the input string
    s = s.replace('\n', '').replace(';','').replace('  ', ' ').replace('ambo i lati', '')
    ...
    #remove \n, - and ; from location
    schedule['location'] = s.split(' –')[1].replace('\n', '').replace('-', '').replace(';', '').replace(r'\s\s', ' ').replace(', ', '').replace(':', '').replace('\u2019', "'").split('COMUNE DI LUCCAUProtocollo')[0].split('e in Via')[0].split('e Piazza')[0].split('e Corso')[0].strip()
    ...
    if len(schedule['location']) > 1:
        # capitalize first letter of location
        schedule['location'] = schedule['location'][0].upper() + schedule['location'][1:]
    ...
    if 'e in Via ' in s or 'e in Piazza ' in s or 'e Piazza' in s or 'e Corso' in s:
        ...
        second['street'] = to_add + ' ' + s.split(split_by)[1].split('–')[0].split('-')[0].strip()

        if 'tratto' in second['street']:
            second['schedule'][0]['location'] = 'tratto' + second['street'].split('tratto')[1];
            # capitalize first letter of location
            second['schedule'][0]['location'] = second['schedule'][0]['location'][0].upper() + second['schedule'][0]['location'][1:]
